...
from data_loader.idc_data_loader import IDCDataLoader
import os
import idc_aug

cached_dataset_path = '/data_large/readonly'
...
class EvaluatorUtils:
    ...
    def evaluate_synset_dataset(it_eval, net, dst_train, testloader, args, logging):
        ...
        for ep in range(Epoch + 1):
            loss_train, acc_train = EvaluatorUtils.epoch('train',
                                                         trainloader,
                                                         net,
                                                         optimizer,
                                                         criterion,
            ...
            if ep % args.eval_gap == 0:
                ...
                _, acc_test = EvaluatorUtils.epoch('test',
                                                   testloader,
                                                   net,
                                                   optimizer,
                                                   criterion,
                ...
                print(
                    ...
                    % (get_time(), it_eval, ep, int(time_train), loss_train, acc_train, acc_test))
        time_train = time.time() - start
        _, acc_test = EvaluatorUtils.epoch('test',
                                           testloader,
                                           net,
    ...
    def epoch(mode, dataloader, net, optimizer, criterion, args, aug, ep, logging):
        ...
        for i_batch, datum in enumerate(dataloader):
            ...
            if args.aug == 'idc' and mode == 'train':
                ...
                r = np.random.rand(1)
                if r < mixup_p:
                    img, targets_a, targets_b, lam = idc_aug.cutmix(img, lab, alpha)
                    output = net(img)
                    loss = idc_aug.mixup_criterion(criterion, output, targets_a, targets_b, lam)
                else:
                    output = net(img)
            ...
            if lab.dtype == torch.float:
                acc = np.sum(
                    np.equal(np.argmax(output.cpu().data.numpy(), axis=-1),
                             np.argmax(lab.cpu().data.numpy(), axis=1)))
            else:
                acc = np.sum(
                    np.equal(np.argmax(output.cpu().data.numpy(), axis=-1),
                             lab.cpu().data.numpy()))
    ...
    def augment(images, dc_aug_param, device):
        if dc_aug_param != None and dc_aug_param['strategy'] != 'none':
            ...
            def rotatefun(i):
                ...
                images[i] = torch.tensor(im_[:, r:r + shape[-2], c:c + shape[-1]],
                                         dtype=torch.float,
                                         device=device)
    ...
    def DiffAugment(x, strategy='', seed=-1, param=None):
        AUGMENT_FNS = {
            'color': [
                EvaluatorUtils.rand_brightness, EvaluatorUtils.rand_saturation,
                EvaluatorUtils.rand_contrast
            ],
    ...
    def get_dataset(args):
        ...
        if args.dataset == 'CIFAR10':
            ...
            dst_train = datasets.CIFAR10(cached_dataset_path,
                                         train=True,
                                         download=True,
                                         transform=transform)
            dst_test = datasets.CIFAR10(cached_dataset_path,
                                        train=False,
                                        download=True,
                                        transform=transform)
        elif args.dataset == 'CIFAR100':
            ...
            if args.normalize_data:
                ...
                transform = transforms.Compose([transforms.ToTensor()])
            dst_train = datasets.CIFAR100(cached_dataset_path,
                                          train=True,
                                          download=True,
                                          transform=transform)
            dst_test = datasets.CIFAR100(cached_dataset_path,
                                         train=False,
                                         download=True,
                                         transform=transform)
        elif args.dataset == 'tinyimagenet':
            ...
            if args.normalize_data:
                ...
                transform = transforms.Compose([transforms.ToTensor()])
            dst_train = datasets.ImageFolder(os.path.join(
                '/nfs/data/justincui/data/tiny-imagenet-200', "train"),
                                             transform=transform)
            dst_test = datasets.ImageFolder(os.path.join(
                '/nfs/data/justincui/data/tiny-imagenet-200', "val", "images"),
                                            transform=transform)
        dst_test = torch.utils.data.DataLoader(dst_test,
                                               batch_size=256,
                                               shuffle=False,
                                               num_workers=0)
    ...
    def get_data_loader(method, factor=2):
        ...
        if method == 'dc':
            ...
            return IDCDataLoader(factor=factor)
    ...
    def get_data_file_name(method, dataset, ipc, factor=2):
        ...
        if method == 'dc':
            ...
            return IDCDataLoader.get_data_file_name(method, dataset, ipc, factor)
        else:
            return ''
