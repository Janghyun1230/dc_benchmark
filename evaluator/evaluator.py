...
DATA_DIR = '/home/janghyun/Codes/datasets'
...
class Evaluator:
    ...
    def evaluate(self, args):
        ...
        if args.method == 'whole':
            _, _, test_accuracy = EvaluatorUtils.evaluate_synset_dataset(
                0, model, self.dst_train, self.dst_test, args, logging)
        else:
            _, _, test_accuracy = EvaluatorUtils.evaluate_synset(0, model, self.train_images,
                                                                 self.train_labels, self.dst_test,
                                                                 args, logging)
...
def prepare_args():
    ...
    parser.add_argument('--eval_gap',
                        type=int,
                        default=1001,
                        help='run evaluation every eval_gap epochs.')
    ...
    parser.add_argument('--dsa_strategy',
                        type=str,
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(
        EvaluatorUtils.pick_gpu_lowest_memory()) if args.gpu == 'auto' else args.gpu
    args.dc_aug_param = EvaluatorUtils.get_daparam(
...
if __name__ == '__main__':
    ...
    print(
        "%s: final acc is: %.2f +- %.2f, dataset: %s, IPC: %d, DSA:%r, num_eval: %d, aug:%s , model: %s, optimizer: %s"
        % (args.method.upper(), mean * 100, std * 100, args.dataset, args.ipc, args.dsa,
           args.num_eval, args.aug, args.model, args.optimizer))
