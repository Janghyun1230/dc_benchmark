...
def rand_bbox(size, lam, cx=None, cy=None):
    W = size[2]
    H = size[3]
...
def random_indices(n, y, nclass=10, intraclass=False, device='cuda'):
    if intraclass:
        assert n == len(y)
        ...
        for c in range(nclass):
            ...
            index_c_perm = index_c[randidx]
            index[y == c] = index_c_perm
    else:
        index = torch.randperm(n).to(device)
    return index


def input_mixup(x, y, alpha=1.0, lam=None, intraclass=False):
    n = len(x)
    if lam is None:
        lam = np.random.beta(alpha, alpha)
    index = random_indices(n, y, intraclass=intraclass)
...
def cutmix(x, y, alpha=1.0, lam=None, cx=None, cy=None, intraclass=False, mask=None):
    n = len(x)
    if lam is None:
        lam = np.random.beta(alpha, alpha)
    index = random_indices(n, y, intraclass=intraclass)
    ...
    if mask is None:
        bbx1, bby1, bbx2, bby2 = rand_bbox(x.size(), lam, cx=cx, cy=cy)
        mask = torch.ones_like(x).detach()
        mask[:, :, bbx1:bbx2, bby1:bby2] = 0
    mixed_x = mask * x + (1. - mask) * x[index]

    if y is not None:
        lam = mask[0, 0].sum() / (x.size()[-1] * x.size()[-2])
        y_a, y_b = y, y[index]
        return mixed_x, y_a, y_b, lam
    else:
        return mixed_x
...
def mixing_fn(x, mixup_type='input', lam=0.5, cx=None, cy=None):
    if mixup_type == 'input':
        mixed_x = input_mixup(x, None, lam=lam)
    elif mixup_type == 'cut':
        mixed_x = cutmix(x, None, lam=lam, cx=cx, cy=cy)
